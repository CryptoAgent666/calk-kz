...
> ⚠️ Сайт размещён на **shared hosting (Plesk, nginx + Apache)**, а **не** на Netlify/Vercel.

- Путь на сервере: `~/calk.kz/`
- **Web-сервер: nginx** (проксирует к Apache)
- `.htaccess` — работает для Apache, но nginx обрабатывает 404 раньше
- Деплой: собранная папка `dist/` загружается на сервер по FTP/SSH
- `netlify.toml` — **не используется** в продакшене

### Настройка 404 в Plesk
В Plesk для домена `calk.kz` нужно указать кастомную 404 страницу:
1. Plesk → Домены → calk.kz → **Настройки Apache и nginx**
2. В секции **nginx** → дополнительные директивы добавить:
   ```
   error_page 404 /404.html;
   ```
3. Или: Plesk → Домены → calk.kz → **Ошибки веб-сервера** → 404 → указать `/404.html`, оставлен для справки
