...
Визуальный проход Data Currency по 20 новым страницам. Деплой не выполнялся.

- **Пеня.** «ст. 121 НК РК» и источник «ст. 117» (старый кодекс) → ст. 5 пп. 2 (кратность 1,25 / 0,65 базовой ставки) и ст. 85 НК-2026. Правка в быстром ответе, методике, i18n ru/kk и источниках; суммы в примерах — «100 000 ₸».
- **Закят (ru + kk).**
  - Нисаб в примерах был зашит как 595 г × 400 ₸ = 238 000 ₸ при 511 700 ₸ в виджете — теперь от тех же цен по умолчанию.
  - Убраны «₸ ₸», добавлены отсутствовавшие ключи `mortgage` / `loan` / `totalDebts` (сырые ключи были видны на обоих языках).
  - Пример 3 поднят выше нисаба.
- **Госномера.** Быстрый ответ («777 — до 2 млн ₸, до 5 млн, аукцион ercep.kz») приведён к прейскуранту 2,8 / 10 / 15–285 МРП.
- **ЕНТ.** Пороги конкурса на грант 2026: педагогика и право 75, здравоохранение 70, национальные вузы 65, остальные 50 — в FAQ, быстром ответе и методике.
- **Рефинансирование.** Примеры пересчитаны по аннуитету (экономия ~7,5 млн и ~260 000 ₸ вместо 3,2 млн и 220 000); формат комиссий «25 000–50 000 ₸».
- **Остальные страницы:**
  - зарплата — в методике потолок ОПВ 50 МЗП (код его уже учитывал);
  - отопление — пример 150 м² «~7,7 т угля» вместо «~6,4 т»;
  - напольные покрытия — «~5 000 ₸/м²» вместо «/упаковка»;
  - сон — нормы NSF/AASM вместо «ВОЗ», «лечь в 23:15»;
  - оценка квартиры — Шымкент 350–650 тыс. ₸/м² во всех копиях;
  - моральный вред — гонорар юриста 100–300 тыс. ₸ вместо «100 000 — 100 000».
- **Реестр.** `penalty_tax_daily_rate_example` (ст. 5 и 85), новый `ent_grant_threshold_scores_2026`.
...
- **Выложено** (бандл 20260924112427, OTA). На проде: 302 URL + ассеты → 200, мусорные URL → 404.
  ⚠️ Выяснилось, что nginx (Plesk) отдаёт статику по GET мимо Apache: HSTS, nosniff, X-Frame-Options, Referrer-/Permissions-Policy и CSP
  из `.htaccess` на пререндеренных страницах до браузера не доходят (так было и раньше; `curl -I` = HEAD идёт в Apache и это маскирует).
  Нужны `add_header … always` в «Additional nginx directives» Plesk.
