...
export default function TimeToWordsCalculator() {
  ...
  const clearAll = () => {
    // results — производное от inputTime (useMemo), пустой ввод даст пустой результат
    setInputTime('');
  };
  ...
  // Запись в историю — побочный эффект (Date.now/Math.random), остаётся в useEffect.
  useEffect(() => {
    if ((window as unknown as { __PRERENDER__?: boolean }).__PRERENDER__) return; // история не должна попадать в статику
    if (results.words && !results.words.includes(t('time-to-words.invalidFormat').substring(0, 10))) {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <QuickAnswer calculatorId="time-to-words" />
      <div className="mb-8">
