...
//   зона ядерных испытаний (Семипалатинск) — 170; зона ЯИ + осложнённые/многоплодные — 184.
type BirthType = 'normal' | 'complicated' | 'zone' | 'zoneComplicated';
const BIRTH_DAYS: Record<BirthType, number> = { normal: 126, complicated: 140, zone: 170, zoneComplicated: 184 };
...
export default function MaternityBenefitsCalculator() {
  ...
  const OPV_RATE = 0.10;
  const maternityDays = BIRTH_DAYS[birthType];
  const MATERNITY_COEFFICIENT = maternityDays / 30; // напр. 126/30 = 4.2
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {t('maternity-benefits.birthTypeLabel')}
                </label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {BIRTH_TYPE_ORDER.map((bt) => (
                    <button
                      key={bt}
                      onClick={() => setBirthType(bt)}
                    ...
                    >
                      <div className="text-sm font-medium">{t(`maternity-benefits.birthType.${bt}`)}</div>
                      <div className="text-xs text-gray-500">
                        {BIRTH_DAYS[bt]} {t('maternity-benefits.daysShort')} · ×{(BIRTH_DAYS[bt] / 30).toFixed((BIRTH_DAYS[bt] * 10) % 30 === 0 ? 1 : 3)}
                      </div>
                    </button>
                  ))}
                </div>
                <p className="text-xs text-gray-500 mt-1">{t('maternity-benefits.birthTypeHint')}</p>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="border border-gray-200 rounded-lg p-4">
                ...
                <p className="text-xs text-gray-500 mb-3">
                  {t(`maternity-benefits.birthType.${birthType}`)}: {maternityDays} {t('maternity-benefits.daysShort')} (×{MATERNITY_COEFFICIENT.toFixed(MATERNITY_COEFFICIENT % 1 === 0 || (MATERNITY_COEFFICIENT * 10) % 1 === 0 ? 1 : 3)})
                </p>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-blue-50 rounded-lg p-5 border border-blue-100">
            ...
            <div className="space-y-3 text-sm text-blue-800">
              ...
              <div className="p-3 bg-white rounded-lg border border-blue-200">
                <div className="font-semibold text-blue-900">{t('maternity-benefits.nuclearZone')}</div>
                <div className="text-lg font-bold text-blue-600 mt-1">{t('maternity-benefits.days170')}</div>
                <div className="text-xs">{t('maternity-benefits.days170Breakdown')}</div>
              </div>
            </div>
          </div>
        </div>
